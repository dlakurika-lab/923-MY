...
if ($count > 0) {
    do {
        echo "<li>(id={$track[$i]['id_track']}) {$track[$i]['name']}</li>\n";
        $i++;
    } while ($i < $count);
}
echo "</ul>\n";
...
for ($i = 0; $i < count($track); $i++) {
    echo "<li>(id={$track[$i]['id_track']}) {$track[$i]['name']} (id альбома: {$track[$i]['id_album']})</li>\n";
}
echo "</ol>\n";
...
foreach ($track as $item) {
    ...
    echo "<td>" . ($item['note'] ? $item['note'] : '&nbsp;') . "</td>\n";
...
while ($i < count($track)) {
    ...
    echo "<td>" . ($track[$i]['note'] ? $track[$i]['note'] : '&nbsp;') . "</td>\n";
